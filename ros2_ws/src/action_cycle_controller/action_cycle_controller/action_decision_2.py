...
class ActionDecisionActionServer2(Node):
    ...
    action clients and vLLM-compatible inference servers, enabling
    ...
    state information (as a structured JSON) and valid action lists,
    formatting these into prompts for the LLM, and returning single-token
    action predictions. This design ensures low-latency responses suitable for
    real-time robotic applications.

    Architecture:
        - Receives ActionDecision goals containing JSON state and valid actions
        - Parses the JSON state payload and formulates inputs using
            `action_decision_pt` prompt template
        - Queries the ASR Proxy / vanilla vLLM server with OpenAI
            InferenceClient, forwarding ASR metadata via extra_body
    ...
    Key Features:
        ...
        - Integration with vLLM servers and ASR proxies
    ...
    Parameters:
        ...
        log_pred_io_pth (str): Directory path for logging LLM prediction
                              ...
                              performed (default: '')
        inference_server_url (str): Base URL of the vLLM inference server
    ...
    Note:
        Requires a running inference server (like an ASR proxy or vanilla vLLM)
        at the configured URL. The single-token output constraint ensures
        fast inference but requires careful prompt engineering to map complex
        decisions to single characters.
    """

    def __init__(self, **kwargs):
        """Initialize the ActionDecisionActionServer2 node.
        ...
        - Inference client setup for vLLM server communication
        ...
        """
        super().__init__('action_decision_2', **kwargs)
        ...
        self.get_logger().info(
            'ActionDecisionActionServer2 initializing\n'
            'Parameters:\n'
    ...
    async def execute_callback_vllm(self, goal_handle):
        ...
        It expects the state to be a JSON string containing ASR metadata
        (pre, chunks, dyn, metadata).
        ...
        Process Flow:
            1. Extract state JSON and valid actions from goal request
            2. Parse JSON, reconstruct full textual state context
            3. Format input using action_decision_pt prompt template
            4. Send chat completion request to vLLM server (w/ ASR metadata)
            5. Extract single-token prediction from response
            6. Log execution time and optionally save prediction I/O
            7. Return result with predicted action
        ...
        state_json = goal.state
        valid_actions = goal.valid_actions

        # Parse JSON state payload and extract ASR metadata
        try:
            state_dict = json.loads(state_json)
            pre = state_dict.get('pre', '')
            chunks = state_dict.get('chunks', '')
            dyn = state_dict.get('dyn', '')
            asr_metadata = state_dict.get('metadata', {})
            full_state_str = pre + chunks + dyn
        except json.JSONDecodeError:
            self.get_logger().error("ActionDecision2 received invalid JSON state")  # noqa
            # Fallback to assuming the state is purely plain text
            full_state_str = state_json
            asr_metadata = {}

        llm_input = action_decision_pt(full_state_str, valid_actions)

        t0 = time.time()

        # Build extra_body to tunnel the ASR metadata to the proxy
        extra_body = {
        ...
        if asr_metadata:
            extra_body["asr_metadata"] = asr_metadata
...
def main(args=None):
    """Main entry point for the ActionDecisionActionServer2 node.

    Initializes the ROS 2 system, creates an ActionDecisionActionServer2
    ...
    Lifecycle:
        ...
        2. Create ActionDecisionActionServer2 node instance
    ...
    node = ActionDecisionActionServer2()
