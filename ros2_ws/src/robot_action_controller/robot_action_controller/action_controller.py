...
class ActionController(Node):
    ...
    def __init__(self):
        ...
        #  State client
        ##################
        self.state_cli = self.create_client(State, 'get_state')
        self.action_cli = self.create_client(State, 'get_action')
        ...
        self.action_resp_pub = self.create_publisher(String, 'action_responese', 10)

        # Timer for action cycle
        self.timer = self.create_timer(0.1, self.run_action_cycle)
    ...
    def get_current_state(self):
        '''
        '''
        state_future = self.state_cli.call_async(self.state_req)
        state_future.add_done_callback(self.get_action)

    def get_action(self, state_future):
        '''
        '''
        try:
        ...
        action_future = self.action_cli.call_async(self.action_req)
